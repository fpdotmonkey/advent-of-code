...
main = do
  args <- getArgs
  part1DataStr <- readFile (args !! 0)
  part2DataStr <- readFile (args !! 1)
...
part1 dataStr = do
  ...
  total <$> (uncurry $ zipAbsDiff) <$> ap2 sort <$> unzip $ part1Tokens
...
part2Similarity [] _ = 0
part2Similarity (x:xs) y = x * (length (findIndices (\e -> e == x) y)) + part2Similarity xs y
